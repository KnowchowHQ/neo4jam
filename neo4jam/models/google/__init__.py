...
class GeminiAPI:
    def __init__(self, model_name: GEMINI_AVAILABLE_MODELS, system_prompt: str | None = None):
        ...
        api_keys = [os.environ.get("GEMINI_1")]
    ...
    def generate(self, query: Union[str, list]) -> str:
        ...
        try:
            ...
            new_key = self._key_manager.get_key()
            logger.info(f"Switching to new key: {new_key}")
...
class GeminiAPIKeyManager:
    def __init__(self, keys, per_day_quota=500, max_per_min_quota=15, cool_off_period=60):
        ...
        self.min_start_time = time()
        self.key_usage = {key: {'min': 0, 'day': 0, 'last_used': 0} for key in keys}

    def get_key(self):
        logger.debug(f"Usage count: {self.usage_count}")
        logger.debug(f"Key usage: {self.key_usage}")
        # Check if it's a new day
        if time() - self.day_start_time > 24 * 60 * 60:
            self.reset_daily_usage()

        # Check if it's a new minute
        if time() - self.min_start_time > 60:
        ...
        if (self.key_usage[current_key]['min'] < self.max_per_min_quota and
            ...
            time() - self.key_usage[current_key]['last_used'] >= self.cool_off_period):
            self.key_usage[current_key]['min'] += 1
            self.key_usage[current_key]['day'] += 1
            self.key_usage[current_key]['last_used'] = time()
            self.usage_count += 1
            return current_key
        else:
            ...
            sleep(60) # - (time() - self.min_start_time))
            return self.get_key()
