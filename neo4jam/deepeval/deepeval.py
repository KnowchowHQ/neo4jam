...
def evaluate_generated_cyphers(
    file_real: str, file_gen: str, rouge_metric, bleu_metric
) -> dict:
    ...
    with open(file_gen, "r") as f:
        data_gen = f.read()
    rougue_score = rouge_metric.calculate(
        predictions=[data_gen], references=[data_real]
    )
...
def deepeval(input_dir: DirectoryPath, output_dir: DirectoryPath) -> None:
    ...
    def job_generator():
        for real, generated in data:
            yield (real, generated, output_dir, rouge_metric, bleu_metric)
...
def __deepeval__(
    ...
    logger.debug("Evaluating {} and {}", real_file, gen_file)
